...
public class MarksDAO implements I_MarksDAO {
    ...
    public boolean addMarks(Marks marks) throws SQLException {
        ...
        PreparedStatement statement = connection.prepareStatement("INSERT INTO marks " +
                "(id, date, studycourse_id, lessons_id, student_id, mark) VALUES (?, ?, ?, ?, ?, ?)");
        statement.setInt(1, marks.getId());
        statement.setDate(2, new java.sql.Date(marks.getDate().getTime())); /** преобразование даты из java.util.Date в java.sql.Date */
        statement.setInt(3, marks.getStudycourse_id());
        statement.setInt(4, marks.getLesson_id());
        statement.setInt(5, marks.getStudent_id());
        statement.setInt(6, marks.getMark().getIntValue());
